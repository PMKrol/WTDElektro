...
public class ameduino_pmkrol1 extends PApplet {

//Ta wersja jest ok z ameduino_lu2
//v3. ukryte niepotrzebne Fx (preskaler itp.)
//v4. dodana średnie VDC (5s.)
...
float trigLevelLPF=.0001f;     // trigLevel lowpass filter (exponential moving average)
...
public void keyPressed() {  
  if (key == CODED) {
    if (connected) {
      ...
      if (keyCode == KeyEvent.VK_F5) {
        if (trigLevelLPF<.01f) trigLevelLPF*=10;
      }
      if (keyCode == KeyEvent.VK_F6) {
        if (trigLevelLPF>.000001f) trigLevelLPF/=10;
      }
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "ameduino_pmkrol1" };
    if (passedArgs != null) {
